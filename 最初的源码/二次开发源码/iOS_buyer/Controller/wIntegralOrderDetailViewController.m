//
//  wIntegralOrderDetailViewController.m
//  My_App
//
...
@interface wIntegralOrderDetailViewController (){
...
@implementation wIntegralOrderDetailViewController
...
-(void)BtnCancelClicked{
    [OHAlertView showAlertWithTitle:@"系统提示" message:@"您要取消该订单？" cancelButton:nil otherButtons:@[@"确定",@"取消"] buttonHandler:^(OHAlertView *alert, NSInteger buttonIndex) {
        if (buttonIndex == 0){
            ...
            [request_2 setDidFailSelector:@selector(my3_urlRequestFailed:)];
            [request_2 setDidFinishSelector:@selector(my3_urlRequestSucceeded:)];
...
-(void)MyBtnClicked{
    onlinePayTypesIntegralViewController *oo = [onlinePayTypesIntegralViewController sharedUserDefault];
    oo.MyBool = YES;
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    if (indexPath.row == 0) {
        ...
        if (dataArray.count!=0) {
            ...
            cell.shipprice.text = [NSString stringWithFormat:@"    运费: %@",class.dingdetail_ship_price];
...
-(void)btnGoodsClicked:(UIButton *)btn{
    if(dataArray.count!=0){
        ...
        exc.ig_id = [[class.goods_groupinfos objectAtIndex:btn.tag-100] objectForKey:@"goods_id"];
