...
- (void)viewDidLoad {
    ...
    imgItem = [[UIImageView alloc]initWithFrame:CGRectMake(0, ScreenFrame.size.height-64-64, ScreenFrame.size.width, 64)];
    imgItem.backgroundColor = [UIColor whiteColor];//SHJ_COLOR;
    imgItem.userInteractionEnabled = YES;
    [self.view addSubview:imgItem];
    
    UILabel *lblAllSelect = [[UILabel alloc]initWithFrame:CGRectMake(45, 22, 40, 20)];
    ...
    btnAllSelect.frame = CGRectMake(0, 10, 40, 40);
...
//    btnAccount.frame = CGRectMake(ScreenFrame.size.width - 100, 17, 80, 30);
    btnAccount.frame = CGRectMake(ScreenFrame.size.width-90,0,90,64);
    btnAccount.tag = 102;
...
- (void)createTableView{
    ...
    if (ScreenFrame.size.height>480) {//说明是5 5s
        shjTableView = [[UITableView alloc]initWithFrame:CGRectMake(0,0, ScreenFrame.size.width, ScreenFrame.size.height-64-64) style:UITableViewStylePlain];
