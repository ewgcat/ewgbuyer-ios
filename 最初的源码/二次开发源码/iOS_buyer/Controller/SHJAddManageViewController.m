...
@interface SHJAddManageViewController ()

@end

@implementation SHJAddManageViewController

- (void)viewWillDisappear:(BOOL)animated
{
    [super viewWillDisappear:YES];
    [requestAddressManagement1 clearDelegatesAndCancel];
    [requestAddressManagement2 clearDelegatesAndCancel];
}
...
- (id)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil
{
    ...
    if (self) {
        ...
        if (ScreenFrame.size.height>480) {//说明是5 5s
            ...
            RegoinTableView = [[UITableView alloc]initWithFrame:CGRectMake(36, 100, ScreenFrame.size.width-36*2, ScreenFrame.size.height-64-49-44-30-10) style:UITableViewStylePlain];
        }else{
            regionView.frame = CGRectMake(0, ScreenFrame.origin.y+12+20, ScreenFrame.size.width, ScreenFrame.size.height);
            RegoinTableView = [[UITableView alloc]initWithFrame:CGRectMake(36, 100, ScreenFrame.size.width-36*2, ScreenFrame.size.height-64-49-44-24) style:UITableViewStylePlain];
        }
        regionView.hidden = YES;
        [self.view addSubview:regionView];
        
        UIImageView *alphView = [[UIImageView alloc]initWithFrame:CGRectMake(regionView.frame.origin.x, regionView.frame.origin.y, regionView.frame.size.width, regionView.frame.size.height)];
        alphView.backgroundColor = [UIColor colorWithRed:64/255.0f green:64/255.0f blue:64/255.0f alpha:1];
        alphView.alpha = 0.7;
        alphView.userInteractionEnabled = YES;
        ...
        [alphView addGestureRecognizer:singleTapGestureRecognizer3];
        
        UILabel *label = [[UILabel alloc]initWithFrame:CGRectMake(36, RegoinTableView.frame.origin.y-30, ScreenFrame.size.width-36*2, 30)];
        label.text = @"请选择城市";
...
-(void)doTimer2{
    [requestAddressManagement1 cancel];
    [requestAddressManagement2 cancel];
    requestAddressManagement1.delegate = nil;
    requestAddressManagement2.delegate = nil;
    [self.navigationController popViewControllerAnimated:YES];
}
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    if (tableView == RegoinTableView) {
        ...
        requestAddressManagement2=[ASIFormDataRequest requestWithURL:url2];
        [requestAddressManagement2 setPostValue:cla.region_id forKey:@"id"];
        
        [requestAddressManagement2 setRequestHeaders:[LJControl requestHeaderDictionary]];
        requestAddressManagement2.tag = 103;
        [requestAddressManagement2 setDelegate:self];
        [requestAddressManagement2 setDidFailSelector:@selector(urlRequestFailed:)];
        [requestAddressManagement2 setDidFinishSelector:@selector(urlRequestSucceeded:)];
        [requestAddressManagement2 startAsynchronous];
...
-(void)safe{
    if (self.tureNameField.text.length == 0){
        ...
        if ([self isMobileField:_mobileField.text] == NO) {
            ...
            requestAddressManagement1 = [ASIFormDataRequest requestWithURL:url];
            [requestAddressManagement1 setPostValue:[fileContent2 objectAtIndex:3] forKey:@"user_id"];
            [requestAddressManagement1 setPostValue:[fileContent2 objectAtIndex:1] forKey:@"token"];
            [requestAddressManagement1 setPostValue:_zipField.text forKey:@"zip"];
            [requestAddressManagement1 setPostValue:My_id forKey:@"area_id"];
            [requestAddressManagement1 setPostValue:@"" forKey:@"mobile"];
            [requestAddressManagement1 setPostValue:_addInfoText.text forKey:@"area_info"];
            [requestAddressManagement1 setPostValue:_mobileField.text forKey:@"telephone"];
            [requestAddressManagement1 setPostValue:_tureNameField.text forKey:@"trueName"];
            
            [requestAddressManagement1 setRequestHeaders:[LJControl requestHeaderDictionary]];
            requestAddressManagement1.tag = 101;
            [requestAddressManagement1 setDelegate: self];
            [requestAddressManagement1 setDidFailSelector:@selector(my1_urlRequestFailed:)];
            [requestAddressManagement1 setDidFinishSelector:@selector(my1_urlRequestSucceeded:)];
            [requestAddressManagement1 startAsynchronous];
        }
    }
}
