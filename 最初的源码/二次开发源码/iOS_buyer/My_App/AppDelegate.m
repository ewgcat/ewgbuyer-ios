...
- (BOOL)application:(UIApplication *)app openURL:(nonnull NSURL *)url options:(nonnull NSDictionary<NSString *,id> *)options{
    ...
    NSLog(@"URL scheme:%@", [url scheme]);
    NSLog(@"URL query: %@", [url query]);
    if ([[url scheme]isEqualToString:@"iskyshop"]) {
        ...
        return YES;
    }else if ([[url scheme]isEqualToString:@"tencent100424468"]) {
...
-(BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions
{
    ...
    //设置微信AppId，设置分享url，默认使用友盟的网址WXAppId:@"wxd930ea5d5a258f4f" appSecret:@"db426a9829e4b49a0dcac7b4162da6b6"
    [UMSocialWechatHandler setWXAppId:@"wx4b3df472a8cd8a2e" appSecret:@"829174345912c0460937f1254a1e35b0" url:@"http://www.umeng.com/social"];
...
//-(void)addressLIst{
//    //获得Vcard
//    _addressListDic= [[ZCAddressBook shareControl]getPersonInfo];
//    //获得序列索引
//    _addressVCard =[[ZCAddressBook shareControl]sortMethod];
//    
//}
#pragma mark -进入主页
...
-(void)index_nav_request{
    ...
    ASIFormDataRequest *index_nav_request=[ASIFormDataRequest requestWithURL:[NSURL URLWithString:url]];
    NSArray *arrObjc3 = [[NSArray alloc]initWithObjects:verify, nil];
    ...
    [index_nav_request setRequestHeaders:dicMy3];
    
    [index_nav_request setDidFailSelector:@selector(index_nav_analyzeFaild:)];
    [index_nav_request setDidFinishSelector:@selector(index_nav_analyzeSuccess:)];
    index_nav_request.delegate = self;
    [index_nav_request startAsynchronous];
}
-(void)index_nav_analyzeFaild:(ASIFormDataRequest *)request{
    //创建tabbar
    [self createTabbar_image];
    launch_ad_View.hidden = YES;
}
-(void)index_nav_analyzeSuccess:(ASIFormDataRequest *)request{
