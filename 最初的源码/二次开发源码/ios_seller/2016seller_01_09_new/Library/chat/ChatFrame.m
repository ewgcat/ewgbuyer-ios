...
- (void)setChat:(Chat *)chat{
    ...
    if (_showTime){
        ...
        CGSize timeSize = [_chat.addTime sizeWithAttributes:@{NSFontAttributeName:kTimeFont}];
    ...
    CGFloat iconX = 10;
    //    // 2.1 如果是自己发得，头像在右边
    if (![_chat.send_from isEqualToString:@"user"]) {
        iconX = screenW - kIconWH;
    }
    ...
    if (![_chat.send_from isEqualToString:@"user"]) {
        contentX = iconX - contentSize.width - kContentLeft - kContentRight;
    }
