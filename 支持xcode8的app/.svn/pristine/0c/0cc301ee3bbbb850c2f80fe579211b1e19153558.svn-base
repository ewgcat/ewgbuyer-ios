//
//  wlifeGroupOrderDetailViewController.m
//  My_App
//
...
@interface wlifeGroupOrderDetailViewController ()

@end

@implementation wlifeGroupOrderDetailViewController
{
    ASIFormDataRequest *requestwlifeGroupOrderDetail3;
}
...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex{
    if (buttonIndex == 0){
        ...
        requestwlifeGroupOrderDetail3 = [ASIFormDataRequest requestWithURL:url3];
        ...
        [requestwlifeGroupOrderDetail3 setPostValue:[fileContent2 objectAtIndex:3] forKey:@"user_id"];
        [requestwlifeGroupOrderDetail3 setPostValue:[fileContent2 objectAtIndex:1] forKey:@"token"];
        [requestwlifeGroupOrderDetail3 setPostValue:class.goods_oid forKey:@"oid"];
        
        [requestwlifeGroupOrderDetail3 setRequestHeaders:[LJControl requestHeaderDictionary]];
        requestwlifeGroupOrderDetail3.tag = 101;
        requestwlifeGroupOrderDetail3.delegate = self;
        [requestwlifeGroupOrderDetail3 setDidFailSelector:@selector(my1_urlRequestFailed:)];
        [requestwlifeGroupOrderDetail3 setDidFinishSelector:@selector(my1_urlRequestSucceeded:)];
        [requestwlifeGroupOrderDetail3 startAsynchronous];
    }else{
    }
}
...
-(void)MyBtnClicked{
//    onlinePayTypeLifeViewController *ooo = [onlinePayTypeLifeViewController sharedUserDefault];
//    ooo.MyBool = YES;
...
- (void)viewWillDisappear:(BOOL)animated
{
    ...
    [requestwlifeGroupOrderDetail3 clearDelegatesAndCancel];
}
...
-(void)viewWillAppear:(BOOL)animated{
    ...
    [requestwlifeGroupOrderDetail3 setPostValue:log.lifeGroup_oid forKey:@"oid"];
    
    [requestwlifeGroupOrderDetail3 setRequestHeaders:[LJControl requestHeaderDictionary]];
    requestwlifeGroupOrderDetail3.tag = 101;
    requestwlifeGroupOrderDetail3.delegate = self;
    [requestwlifeGroupOrderDetail3 setDidFailSelector:@selector(urlRequestFailed:)];
    [requestwlifeGroupOrderDetail3 setDidFinishSelector:@selector(urlRequestSucceeded:)];
    [requestwlifeGroupOrderDetail3 startAsynchronous];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (dataArray.count!=0) {
        ...
        UILabel *labelLineT = [[UILabel alloc]initWithFrame:CGRectMake(6, 103+16+23, ScreenFrame.size.width, 0.5)];
...
//        [button addTarget:self action:@selector(DEtailBtnClicked) forControlEvents:UIControlEventTouchUpInside];
        ...
        UIView *labelLineT2 = [[UIView alloc]initWithFrame:CGRectMake(6, 28+16+22, ScreenFrame.size.width, 0.5)];
        labelLineT2.backgroundColor = [UIColor grayColor];
        ...
        if ([class.dingdetail_payType isEqualToString:after_mark]) {
            ...
            if ([class.goods_status intValue] == 20) {
                ...
                buttonZ.frame =CGRectMake(230, 20, 80, 30);
        ...
        for(int i=0;i<class.goods_groupinfos.count;i++){
            UIView *viewGray = [[UIView alloc]initWithFrame:CGRectMake(10, 90+76*i+30+16+20, ScreenFrame.size.width-20, 66)];
        ...
        UIView *labelLineT3 = [[UIView alloc]initWithFrame:CGRectMake(6, 90+76*class.goods_groupinfos.count+30+16+20, ScreenFrame.size.width, 0.5)];
        ...
        UILabel *labelTP = [[UILabel alloc]initWithFrame:CGRectMake(0, 90+76*class.goods_groupinfos.count+30+16+4+20, 300, 24)];
