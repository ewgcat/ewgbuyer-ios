...
-(void)getGoodsSuits:(NSString *)goodId{
    ...
    [request setPostValue:[SYObject currentUserID] forKey:@"user_id"];
    [request setPostValue:[SYObject currentToken] forKey:@"token"];
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section{
    ...
    UILabel *marketPrice=[LJControl labelFrame:CGRectMake(CGRectGetMaxX(pice.frame)+5, 5, 100, 30) setText:@"" setTitleFont:17 setbackgroundColor:[UIColor clearColor] setTextColor:[UIColor darkGrayColor] textAlignment:NSTextAlignmentLeft];
    ...
    if (tableView==_suiteTableView) {
        ...
        NSString *marketText=[NSString stringWithFormat:@"￥%.2f",[[dict objectForKey:@"all_price"] floatValue]];
        NSMutableAttributedString *attri = [[NSMutableAttributedString alloc] initWithString:marketText];
        [attri addAttribute:NSStrikethroughStyleAttributeName value:@(NSUnderlinePatternSolid | NSUnderlineStyleSingle) range:NSMakeRange(0, marketText.length)];
        [attri addAttribute:NSStrikethroughColorAttributeName value:[UIColor darkGrayColor] range:NSMakeRange(0, marketText.length)];
        [marketPrice setAttributedText:attri];
        
        
    }else  if (tableView==_partsTableView){
        NSDictionary *dict=[_partsArray objectAtIndex:section];
        title.text=[NSString stringWithFormat:@"套餐%ld",(long)section+1];
        pice.text=[NSString stringWithFormat:@"￥%.2f",[[dict objectForKey:@"all_price"] floatValue]];
...
#pragma mark - 这里我添加了一下参考价
    
    [view addSubview:marketPrice];
