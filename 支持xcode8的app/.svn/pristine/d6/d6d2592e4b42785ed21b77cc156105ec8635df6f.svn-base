...
#pragma mark --这里的网络请求原本是在viewwillappear的
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1.0 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        if (status.integerValue!=9000) {
            ...
            dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1.0 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                ...
                }else if([_order_type isEqualToString:@"ios"] || [[d valueForKey:@"mlh_order_type"] isEqualToString:@"ios"]){
//                    [SYObject failedPrompt:@"升级成功！"];
//                    [self.navigationController popToRootViewControllerAnimated:YES];
//                    [SYObject redirectvip:self.navigationController nameNphone:@"" address:@"" realprice:th.jie_order_goods_price];
                    //如果是vip升级的话就直接跳到个人中心那里
                    [d setObject:@"" forKey:@"mlh_order_type"];
                    [d synchronize];
                    [self.navigationController popToRootViewControllerAnimated:YES];
                }else if ([_order_type isEqualToString:@"integral"] || [[d valueForKey:@"mlh_order_type"] isEqualToString:@"integral"]){
                    //支付宝积分购买成功的
                    [SYObject integralAfterPayment:self.navigationController orderNum:@{@"orderNum":th.ding_hao,@"price":th.jie_order_goods_price} orderType:@"integral"];
...
#pragma mark - 这里有问题的代码
//        NSUserDefaults *d=[NSUserDefaults standardUserDefaults];
//       NSString *count= [d valueForKey:@"buygoodcount"];
//        
//        
//        NSUserDefaults *a=[NSUserDefaults standardUserDefaults];
//     NSString *price=  [a valueForKey:@"realPrice"];
        
        NSLog(@"===尾款==%@",th.jie_order_goods_price);
        float goodPrice=[th.jie_order_goods_price floatValue];
        lableJM.text = [NSString stringWithFormat:@"￥%0.2f",goodPrice];
//        lableJM.text=price;
...
-(IBAction)selectBtnClicked:(id)sender{
    ...
    if (tag==10000 || tag==10001 ||tag==10003) {//微信的，支付宝的，银联的
        if (!_currentBtn) {//如果已选中的按钮为空，则点击后将当前的按钮赋值给已选中的按钮。
            _currentBtn = btn;
        ...
        else if (_currentBtn!=btn) {//如果已选中的按钮不是当前选中的按钮，则
            NSInteger index = [_btnArray indexOfObject:_currentBtn];
            UIImageView *curCB = _cbArray[index];
            ...
            UIImageView *curCB = _cbArray[index];
            curCB.highlighted=!curCB.highlighted;
            tagZHI = tag-10000;
        }

    }else if(tag==10002){//预存款的
        NSInteger index = [_btnArray indexOfObject:sender];
        UIImageView *curCB = _cbArray[index];
        curCB.highlighted=!curCB.highlighted;
//        [curCB setHighlighted:NO];
...
//            NSDictionary *dic = [dataArray objectAtIndex:tagZHI];
            NSMutableArray *payStyle=[NSMutableArray array];
            [_cbArray enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
                
                UIImageView *i=obj;
                NSLog(@"===%zd",i.highlighted);
                if (i.highlighted==YES) {
                    NSString *pay=dataArray[idx][@"pay_mark"];
                    [payStyle addObject:pay];
                    NSLog(@"pay==%@",pay);
                    
                    NSLog(@"payStyleArr==%@",payStyle);

                }
            }];
            
            if ([payStyle containsObject:balance_mark]) {//如果有预存款
                //提前判断是否设置了支付密码
                [SYObject startLoading];
                [SYShopAccessTool checkPswdNMobile:^(BOOL has) {
                    if (!has) {
                        ...
                        NSLog(@"_order_type_order_type==%@",_order_type);
                        
#pragma mark -这里为了优化代码，不使用单例，而是将order_type 存在本地，因为这里只有混合支付的时候有点小问题，
                        NSUserDefaults *d=[NSUserDefaults standardUserDefaults];

                        if (_order_type) {
                            
                            [d setObject:_order_type forKey:@"mlh_order_type"];
                            [d synchronize];
...
#pragma mark -设置支付方式的数组
                        obTVC.payStyleArr=payStyle;
            ...
            }else if ([payStyle containsObject: online_mark]) {//支付宝支付
                //这里指定支付类型
            ...
            }else if ([payStyle containsObject:weixin_mark]){//微信支付
                //这里指定支付方式:
            ...
            }else if ([payStyle containsObject: unionpay_mark]){//银联支付
                if (_order_type) {
                }else{
                    _order_type = @"goods";
                }
...
#pragma mark 这里修改了MyTableView的创建，不然会被创建多次的
