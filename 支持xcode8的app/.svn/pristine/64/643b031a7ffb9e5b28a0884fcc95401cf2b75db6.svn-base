...
//    ASIFormDataRequest *request_17;
    ...
    ASIFormDataRequest *request_21;//这个请求好像没有用
    ASIFormDataRequest *request_22;//赠品点请求接口，因为没有用到，所以先不替换
    UIView *networkView;
    UIView *bottomView;
...
-(void)viewWillAppear:(BOOL)animated{
    ...
    if([fileManagerDong fileExistsAtPath:filePathDong]==NO){
        logView.hidden = NO;
        MyTableView.frame = CGRectMake(0, 44, ScreenFrame.size.width, ScreenFrame.size.height-49-56-64-44);
        labelWu.frame = CGRectMake(0,49, ScreenFrame.size.width, ScreenFrame.size.height);
        
    }else{
        logView.hidden = YES;
        MyTableView.frame = CGRectMake(0, 0, ScreenFrame.size.width, ScreenFrame.size.height-49-56-64);
...
- (void)viewDidLoad {
    ...
    MyTableView = [[UITableView alloc]initWithFrame:CGRectMake(0, 0, ScreenFrame.size.width, ScreenFrame.size.height-49-56-64) style:UITableViewStyleGrouped];
    ...
    labelWu = [[UIView alloc]initWithFrame:CGRectMake(0, 0, ScreenFrame.size.width, ScreenFrame.size.height)];
    ...
    bottomView.frame = CGRectMake(0, ScreenFrame.size.height-49-56-64, ScreenFrame.size.width, 56);
    ...
    btnQ.frame = CGRectMake(0, ScreenFrame.size.height-49-56-64+8, 40, 40);
    btnQ.tag = 101;
    [btnQ addTarget:self action:@selector(allSelected:) forControlEvents:UIControlEventTouchUpInside];
    [self.view addSubview:btnQ];
   
    UILabel *lae3 = [[UILabel alloc]initWithFrame:CGRectMake(35,12, 40, 32)];
    lae3.text = @"全选";
    lae3.textColor = [UIColor whiteColor];
...
//    [bottomView addSubview:imageLineTop1];
    ...
    _jiesuan.font = [UIFont boldSystemFontOfSize:15];
...
-(void)cardClicked:(UIButton *)btn{
    ...
    NSLog(@"specIdsString=%@~~cartId=%@",specIdsString,cartId);
...
-(void)carShoppingRefresh{
    ...
    if([fileManagerDong fileExistsAtPath:filePathDong]==NO){
       ...
       NSString *urlStr= [NSString stringWithFormat:@"%@%@",FIRST_URL,MYCAR_URL];
        ...
        if([fileManagerDong fileExistsAtPath:filePathDong]==NO){
            ...
            [par setObject:@"" forKey:@"cart_mobile_ids"];
        }else{
            NSString *readPath2 = [documentsPath stringByAppendingPathComponent:@"cart_mobile_id.txt"];
            NSArray *fileContent2 = [[NSArray alloc] initWithContentsOfFile:readPath2];
            [par setObject:[fileContent2 objectAtIndex:0]  forKey:@"cart_mobile_ids"];
        }
        ...
        [[Requester managerWithHeader]POST:urlStr parameters:par success:^(AFHTTPRequestOperation *operation, id responseObject) {
            [ws my12_urlRequestSucceeded:responseObject];
        ...
        __weak typeof(self) ws=self;
        [[Requester managerWithHeader]POST:urlStr parameters:par success:^(AFHTTPRequestOperation *operation, id responseObject) {
            [ws my13_urlRequestSucceeded:responseObject];
        } failure:^(AFHTTPRequestOperation *operation, NSError *error) {
            NSLog(@"%@",[error localizedDescription]);
            NSLog(@"\n\n***错误信息:***\n*类名:%@*\n*方法名:%s*\n*行数:%d*\n\n",NSStringFromClass([self class]),__func__,__LINE__);
            [ws urlRequestFailed];
...
-(void)my12_urlRequestSucceeded:(NSDictionary *)dic{
//    int statuscode2 = [request responseStatusCode];
//    if (statuscode2 == 200) {
    NSDictionary *dicBig =dic;
    NSLog(@"dicBigdicBig==%@",dicBig);
...
-(void)my13_urlRequestSucceeded:(NSDictionary *)dic{
//    int statuscode2 = [request  responseStatusCode];
//    if (statuscode2 == 200) {
    NSDictionary *dicBig =dic;
...
-(void)settle_accounts{
    ...
    NSFileManager *fileManagerDong = [NSFileManager defaultManager];
    if([fileManagerDong fileExistsAtPath:filePathDong]==NO){
        [OHAlertView showAlertWithTitle:@"温馨提示" message:@"您还未登录，无法结算" cancelButton:nil otherButtons:@[@"去登录"] buttonHandler:^(OHAlertView *alert, NSInteger buttonIndex) {
            if(buttonIndex==0){
                NewLoginViewController *new = [[NewLoginViewController alloc]initWithNibName:@"NewLoginViewController" bundle:nil];
                [self.navigationController pushViewController:new animated:YES];
            }else{
        ...
        if ([_jiesuan.text intValue]>0) {
            ...
            for(int i=0;i<arrGG.count;i++){
                ...
                NSString *SS =[NSString stringWithFormat:@"%@",[arrGG objectAtIndex:i]];
                if (SS.length == 0) {
...
-(void)tableviewRefrsh{
    ...
    NSString *urlStr=[NSString stringWithFormat:@"%@%@",FIRST_URL,CART_PRICE_URL];
    
    
    NSMutableDictionary *par=@{}.mutableCopy;
    if([fileManagerDong fileExistsAtPath:filePathDong] == NO){
        [par setObject:_cart_meideng forKey:@"cart_ids"];
    }else{
        [par setObject:_cart_ids forKey:@"cart_ids"];
    }
    __weak typeof(self) ws=self;
    [[Requester managerWithHeader]POST:urlStr parameters:par success:^(AFHTTPRequestOperation *operation, id responseObject) {
...
#pragma mark - 存储一下总价格
        ...
        NSString *price=[NSString stringWithFormat:@"%@",[dicBig objectForKey:@"select_cart_price"]];
        NSUserDefaults *d=[NSUserDefaults standardUserDefaults];
        [d setObject:price forKey:@"allPrice"];
        [d synchronize];
        
        NSLog(@"allPrice=%@",[d valueForKey:@"allPrice"]);
    ...
    if (statuscode2 == 200) {
        ...
        if([fileManagerDong fileExistsAtPath:filePathDong]==NO){
            ...
            if (arr.count == 1) {
                ...
                NSLog(@"classs222===%@",[[arr objectAtIndex:0] class]);
                
                NSString *str1=[NSString stringWithFormat:@"%@",[arr objectAtIndex:0]];
                NSString *str2=[NSString stringWithFormat:@"%@",[arr objectAtIndex:1]];
            ...
            if(arr.count!=0){
                if (indexPath.row<arr.count) {
                    if([[arr_discount_list objectAtIndex:indexPath.section] rangeOfString:@"combine_"].location !=NSNotFound){
                        ...
                        NSString *namestring=[NSString stringWithFormat:@"%@",[[arr objectAtIndex:indexPath.row] objectForKey:@"goods_name"]];
...
//               [[arr objectAtIndex:indexPath.row] objectForKey:@"goods_count"]
                if (indexPath.row<arr.count) {
                    NSDictionary *dict=[arr objectAtIndex:indexPath.row];
                    
                    
#pragma mark -这里保存一下买的数量
                    
                    NSUserDefaults *d=[NSUserDefaults standardUserDefaults];
                    [d setObject:dict[@"goods_count"] forKey:@"buygoodcountFromBuyCar"];
                    [d synchronize];
                    
                    NSLog(@"goods_count==%@ count=%@",dict[@"goods_count"],[d valueForKey:@"buygoodcountFromBuyCar"]);
...
-(void)spcClick:(UIButton *)btn{
    ...
    [self getCountAndPrice:specIdsArray];
   ...
   NSDictionary *dicBig =[NSJSONSerialization JSONObjectWithData:request.responseData options:NSJSONReadingMutableContainers error:nil];
    if(dicBig){
        if ([[dicBig objectForKey:@"code"]intValue]==100) {
            ...
            [SYObject failedPrompt:@"团购库存不足"];
        }else if ([[dicBig objectForKey:@"code"]intValue]==400){
            [SYObject failedPrompt:@"商品超出限购数量"];
        }else if ([[dicBig objectForKey:@"code"]intValue]==500){
            [SYObject failedPrompt:@"F码商品只能购买一件"];
        }else{
            [SYObject failedPrompt:@"未知原因导致未能修改成功"];
        }
        [MyTableView reloadData];
    ...
    if (indexpath.section==0 && indexpath.row==0) {
        ...
        [MyTableView scrollToRowAtIndexPath:indexpath atScrollPosition:UITableViewScrollPositionTop animated:YES];
            [UIView animateWithDuration:1 animations:^{
                self.view.frame=CGRectMake(0,-20, ScreenFrame.size.width, ScreenFrame.size.height);
            }];
    }else{
        [MyTableView scrollToRowAtIndexPath:indexpath atScrollPosition:UITableViewScrollPositionTop animated:YES];
    }
...
//    buttonDelete = nil;
    ...
    _select_order_id = nil;
    _LongorderInfo = nil;
    _onlineArray = nil;
    _usecouponsMoney = nil;
    _usecouponsID = nil;
    _train_order_id = nil;
    _xiu_addr_id = nil;
    _xiu_area = nil;
...
#pragma mark -赠品点请求，因为没有用到，所以不换成afn
