    ...
    if (metadataObjects != nil && [metadataObjects count] > 0) {
        ...
        if ([[metadataObj type] isEqualToString:AVMetadataObjectTypeQRCode]) {
            ...
            if([_scanStr rangeOfString:@"qr_session_id="].location !=NSNotFound)
            {
               ...
               // [self dismissViewControllerAnimated:YES completion:nil];
//                 [self.navigationController popViewControllerAnimated:YES];
            }else if ([_scanStr rangeOfString:@"spread"].location !=NSNotFound){
                dispatch_sync(dispatch_get_main_queue(), ^{
//
                    NSString *mainStr= [_scanStr stringByReplacingOccurrencesOfString:@".htm" withString:@""];
                    
                    NSString *inviteCode= [mainStr componentsSeparatedByString:@"/"].lastObject;
                    
                    
                    loginFastViewController *vc=[[loginFastViewController alloc]init];
                    vc.isScanRegister=YES;
