...
@property(nonatomic,strong)UIButton *peisongbut;//配送方式的按钮
@end
...
//            label.text =[NSString stringWithFormat:@"订单中的%@件商品，由%@为您发货",[dic objectForKey:@"goods_count"],[dic objectForKey:@"store_name"]];
                 label.text =[NSString stringWithFormat:@"订单中的商品，由%@为您发货",[dic objectForKey:@"store_name"]];
...
//            [_myscrollview addSubview:imagePic];
            ...
            fei= [[UILabel alloc]initWithFrame:CGRectMake(220, 120, 90, 40)];
        ...
        _quedingBtn.frame = CGRectMake(125, 120+21+73*(i-1), 70, 33);
            ...
            if (btnTag == 0 ) {
                ...
                for(int i=0;i<arr.count;i++){
                    ...
                    if ([[arrmy objectAtIndex:1] isEqualToString:@"卖家承担"]) {
                        ...
                        youfeiMoney = youfeiMoney+[[sss2 objectAtIndex:0] floatValue];
                    }
                    if (str.length == 0) {
                ...
                th.wuliu = [NSString stringWithFormat:@"%.2f",youfeiMoney];
                th.fee = str;
                
                if(sinceComeTag==YES){//自提
                    th.ship_price = @"0";
                    th.trans = @"0";
                    th.fee =  @"自提";
                    th.xuanzeyoufei = 0;
                    th.wuliu = [NSString stringWithFormat:@"%ld",(long)0];
...
-(CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath{
    if (tableView == MyTableView222) {
        if (sinceComeTag == NO) {
            return 166+134*MyDataarray.count+44+44*4+16-16-40+64;
        }else{
...
#pragma mark - 这里把货到付款隐藏掉
                for(int i=0;i<zhifufangshiDataarray.count-1;i++){
                    UIButton *button = [UIButton buttonWithType:UIButtonTypeCustom ];
...
//                label.text =[NSString stringWithFormat:@"订单中的%@件商品，由%@为您发货",[dic objectForKey:@"goods_count"],[dic objectForKey:@"store_name"]];
                ...
                _peisongbut = [UIButton buttonWithType:UIButtonTypeCustom ];
                _peisongbut.frame =CGRectMake(20, 200+134*i, 90, 34);
                _peisongbut.tag = i+500;
                [_peisongbut.layer setMasksToBounds:YES];
                [_peisongbut.layer setCornerRadius:4.0];
                
                if ([_addressStr isEqualToString:@"点击选择自提点"]) {
                    [_peisongbut setTitle:[arrayShip objectAtIndex:i] forState:UIControlStateNormal];

                }else{
                      [_peisongbut setTitle:@"自提" forState:UIControlStateNormal];
                
                }
                [_peisongbut addTarget:self action:@selector(buttonBackGroundHighlighted:) forControlEvents:UIControlEventTouchDown];
                [_peisongbut addTarget:self action:@selector(buttonBackGroundNormal:) forControlEvents:UIControlEventTouchUpInside];
                _peisongbut.titleLabel.font = [UIFont systemFontOfSize:14];
                _peisongbut.backgroundColor =[UIColor orangeColor];
//                _peisongbut.backgroundColor = [UIColor blueColor];

                
                [_peisongbut addTarget:self action:@selector(btnClicked:) forControlEvents:UIControlEventTouchUpInside];
                [cell addSubview:_peisongbut];
...
//                fei= [[UILabel alloc]initWithFrame:CGRectMake(ScreenFrame.size.width-105, 154+21+73*i, 90, 34)];
                  fei= [[UILabel alloc]initWithFrame:CGRectMake(ScreenFrame.size.width-105, 154, 90, 34)];
                fei.font = [UIFont systemFontOfSize:15];
                fei.textColor = [UIColor grayColor];
                fei.tag = i+500;
                ...
                NSArray *arrList = [dic objectForKey:@"transInfo_list"];
                fei.text =[NSString stringWithFormat:@"%@",[[arrList objectAtIndex:0] objectForKey:@"key"]];
                NSLog(@"feifeifei55:%@",fei);
                [_myscrollview2 addSubview:fei];
            }
            
            UIImageView *imageLL2 = [[UIImageView alloc]initWithFrame:CGRectMake(0, 120+134*MyDataarray.count, ScreenFrame.size.width, 16)];
            imageLL2.backgroundColor = RGB_COLOR(246, 246, 246);
            [cell addSubview:imageLL2];
            
            //选择配送时间
            UILabel *shiplabel = [[UILabel alloc]initWithFrame:CGRectMake(20, 146+134*MyDataarray.count, ScreenFrame.size.width-40, 44)];
