...
{
    NSArray *_cart_ids;//购物车的id数组
    
    NSString *_country;//产地
    NSString *_countryPhoto;//产地图片
}
//虚拟
//视图
...
//自营的label
@property (nonatomic, weak)UILabel *ziyinglabel;
...
- (void)viewDidLoad{
    ...
    [d setObject:@"1" forKey:@"buygoodcount"];
    [d synchronize];
    specSelectBool = NO;
    [super viewDidLoad];
...
#pragma mark -如果要给MyTableView即商品显示的tableview，要设置以下方面：
//1.设置heightForRowAtIndexPath
//2.cellForRowAtIndex
//
    ...
    if (tableView == MyTableView) {
        if (indexPath.row == 0) {
            if (dataArray.count!=0) {
                ...
                NSUserDefaults *d=[NSUserDefaults standardUserDefaults];
                NSString *level=[d valueForKey:@"level_name"];
                //如果包括了钻卡会员这个价格的话

                
                
//                if(class.vipprice.length>0 &&(level==nil || (![level isEqualToString:@"钻卡会员"]) || [level isEqualToString:@""])){
                        return ScreenFrame.size.width+20+requiredSize.size.height+20+35 +30;
    ...
    if (tableView == MyTableView) {
        ...
        if (dataArray.count!=0) {
            ...
            if (indexPath.row == 0) {
                ...
                int height=ScreenFrame.size.width-40;
                _myScrollView2 = [[UIScrollView alloc]initWithFrame:CGRectMake(0, 0, ScreenFrame.size.width, height)];
                ...
                _myScrollView2.contentSize=CGSizeMake(ScreenFrame.size.width*arrImage.count,height);
                [cell addSubview:_myScrollView2];
                
                for(int i=0;i<arrImage.count;i++){
                    UIImageView *imageVIew = [[UIImageView alloc]initWithFrame:CGRectMake(ScreenFrame.size.width*i, 0, ScreenFrame.size.width, height)];
                    [imageVIew sd_setImageWithURL: [NSURL URLWithString:[arrImage objectAtIndex:i]] placeholderImage:[UIImage imageNamed:@""]];
                    [_myScrollView2 addSubview:imageVIew];
                }
                pagelabel=[LJControl labelFrame:CGRectMake(ScreenFrame.size.width-60,height-60, 50, 50) setText:[NSString stringWithFormat:@"1/%ld",(long)arrImage.count] setTitleFont:16 setbackgroundColor:UIColorFromRGB(0X808A87) setTextColor:[UIColor whiteColor] textAlignment:NSTextAlignmentCenter];
                pagelabel.alpha=0.8;
                pagelabel.layer.cornerRadius = 25;
                pagelabel.layer.masksToBounds = YES;
                [cell addSubview:pagelabel];
                
                UILabel *ll=[LJControl labelFrame:CGRectMake(10,height, ScreenFrame.size.width-10, 1) setText:@"" setTitleFont:0 setbackgroundColor:UIColorFromRGB(0XF2F2F2) setTextColor:[UIColor clearColor] textAlignment:NSTextAlignmentCenter];
                [cell addSubview:ll];
#pragma mark - 产地
              
                UIImageView *countryimageview=[[UIImageView alloc]init];
                countryimageview.frame=CGRectMake(10, height+10, 50, 30);
              //  countryimageview.backgroundColor=[UIColor redColor];
                countryimageview.contentMode=UIViewContentModeScaleAspectFit;
               [countryimageview sd_setImageWithURL:[NSURL URLWithString:_countryPhoto]];
                [cell addSubview:countryimageview];
                UILabel *countylabel=[[UILabel alloc]initWithFrame:CGRectMake(80, height+10, 200, 30)];
                countylabel.font=[UIFont systemFontOfSize:14];
                countylabel.text=_country;
                [cell addSubview:countylabel];
#pragma mark - 商品名称
                
             
                
                
                UILabel *labelPrice222 = [[UILabel alloc]initWithFrame:CGRectMake(10, ScreenFrame.size.width+20, ScreenFrame.size.width-20, 46)];
...
#pragma mark  价格中的￥符号显示佣金
             
//                money.backgroundColor=[UIColor blackColor];
                money.text = @"￥";
                money.textColor = SY_PRICE_COLOR;
                money.font = [UIFont boldSystemFontOfSize:15];
                [cell addSubview:money];
//                c+ell.backgroundColor=[UIColor yellowColor];
...
#pragma mark  价格显示
                ...
                NSLog(@"detail_goods_current_price===%@",classify.detail_goods_current_price);
                NSLog(@"======$frame==%@",NSStringFromCGRect(labelcurrent_price.frame));

#pragma mark 添加市场价
                NSLog(@"goodprice==%@  maxx= %f x=%f",classify.goodprice,CGRectGetMaxX(labelcurrent_price.frame)+20,labelcurrent_price.frame.origin.x);
                if (classify.goodprice !=nil) {
                    
                    float x=CGRectGetMaxX(labelcurrent_price.frame)+20;
                    int width;
                    if (_ziyinglabel) {//如果有自营的label就限制最大宽度
                      width =CGRectGetMinX(_ziyinglabel.frame)-5-(CGRectGetMaxX(labelcurrent_price.frame)+20);
                    }else{
                        width=100;
                    }
                    
                    UILabel *marketLLabel = [[UILabel alloc]initWithFrame:CGRectMake(x, CGRectGetMinY(labelcurrent_price.frame), width,20)];
                    [cell addSubview:marketLLabel];

                    NSString *str=[NSString stringWithFormat:@"￥%@",classify.goodprice];;
                    
                    marketLLabel.text = str;
                    NSLog(@"======frame==%@",NSStringFromCGRect(marketLLabel.frame));
                    
                    NSDictionary *dict = @{NSStrikethroughStyleAttributeName:[NSNumber           numberWithInteger:NSUnderlineStyleThick], NSStrikethroughColorAttributeName:[UIColor  lightGrayColor], NSForegroundColorAttributeName:[UIColor lightGrayColor] };
                    
                    NSMutableAttributedString *attribtStr = [[NSMutableAttributedString alloc]initWithString:marketLLabel.text];
                    
                    [attribtStr addAttributes:dict range:NSMakeRange(0, str.length)];
                    
                    
                    
                    marketLLabel.attributedText = attribtStr;
...
#pragma mark 添加钻卡价格
                ...
                NSUserDefaults *d=[NSUserDefaults standardUserDefaults];
              NSString *level=[d valueForKey:@"level_name"];
                //如果不是钻卡会员，而且钻卡会员价格不为空
                NSLog(@"level==%@++vipprice=%@",level,classify.vipprice);
                
#pragma mark -这里钻卡会员也显示，
//                if(classify.vipprice.length>0 &&(level==nil || (![level isEqualToString:@"钻卡会员"]) || [level isEqualToString:@""]))
                    {
                        NSLog(@"=====");
                        UILabel *label2 = [[UILabel alloc] initWithFrame:CGRectMake(12, CGRectGetMaxY(labelcurrent_price.frame)+15, 200,20)];
                        
                        label2.font=[UIFont systemFontOfSize:14];
                        label2.text = [NSString stringWithFormat:@"会员价：￥%@",classify.vipprice];;
                        label2.textColor=[UIColor lightGrayColor];
                        
//                        if (classify.vipprice>classify.detail_goods_current_price) {
//
//                        }else{
...
#pragma mark -暂时先隐藏
//                        UIButton *button=[[UIButton alloc]initWithFrame:CGRectMake(ScreenFrame.size.width-110-10, CGRectGetMinY(label2.frame), 110, 25)];
//                        button.titleLabel.font=[UIFont systemFontOfSize:14];
//                        [button setTitle:@"立即升级钻卡" forState:UIControlStateNormal];
//                        
//                        button.layer.cornerRadius=3;
//                        button.layer.masksToBounds=YES;
//                        
//                        [button addTarget:self action:@selector(gotoShenji:) forControlEvents:UIControlEventTouchUpInside];
...
//                            cell.areaSelectionLabel.text=@"区域未知";
                    ...
                    cell.areaSelectionLabel.hidden=YES;
                    
                    UILabel *ll=[LJControl labelFrame:CGRectMake(10,1, ScreenFrame.size.width-10, 1) setText:@"" setTitleFont:0 setbackgroundColor:UIColorFromRGB(0XF2F2F2) setTextColor:[UIColor clearColor] textAlignment:NSTextAlignmentCenter];
                    [cell addSubview:ll];
                    if ([classify.detail_goods_inventory intValue]>0) {
                        cell.cargoVolumeLabel.text = [NSString stringWithFormat:@"库存：现货"];
                    }else {
                        cell.cargoVolumeLabel.text = [NSString stringWithFormat:@"库存：缺货"];
                    }
                    if (feetext.length>0) {
            ...
            if (indexPath.row == 5) {
                if (self.evaluateArray.count>0) {
                    ...
                    [cell.userImgView sd_setImageWithURL:[NSURL URLWithString:model.user_img] placeholderImage:[UIImage imageNamed: @"nouse-mlh" ]];
...
#pragma mark 点击定位图标中的地区的tableview
    if (tableView == RegoinTableView) {
    ...
    if (tableView == zuheTableView){
        ...
        if (zuheArray.count!=0) {
            ...
            for(int j=0;j<arr.count;j++){
                ...
                [image sd_setImageWithURL:[NSURL URLWithString:[[[[zuheArray objectAtIndex:indexPath.row] objectForKey:@"goods_list"] objectAtIndex:j] objectForKey:@"goods_img"]] placeholderImage:[UIImage imageNamed:@""]];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    if (tableView == MyTableView) {
        ...
        if (indexPath.row == 2) {
            ...
            [iamgeView sd_setImageWithURL:[NSURL URLWithString:[arrImage objectAtIndex:0]] placeholderImage:[UIImage imageNamed:@"nouse-mlh"]];
...
#pragma mark -跳到钻卡升级界面、
-(void)gotoShenji:(UIButton *)sender{
...
#pragma mark -获取网络数据
        ...
        //钻石会员价格
        if ([dicBig objectForKey:@"jewel_price"]) {
            
            float a=[[dicBig objectForKey:@"jewel_price"] floatValue];
            classify.vipprice =[NSString stringWithFormat:@"%.2f",a];
...
#pragma mark -添加产地，产地图片
        if (dicBig[@"country"]) {
            _country=dicBig[@"country"];

        }
        if (dicBig[@"country_photo"]) {
            _countryPhoto=dicBig[@"country_photo"];
        ...
        //市场价格
        if ([dicBig objectForKey:@"goods_price"]) {
            classify.goodprice =[NSString stringWithFormat:@"%@",[dicBig objectForKey:@"goods_price"]];
...
#pragma mark -++++++++
            ...
            if ([dicBig objectForKey:@"jewel_price"]) {
                ...
                float a=[[dicBig objectForKey:@"jewel_price"] floatValue];
                class.vipprice =[NSString stringWithFormat:@"%.2f",a];
...
-(void)addCaron_urlRequestSucceeded:(ASIFormDataRequest *)request{
    ...
    if (statuscode2 == 200) {
        ...
        if (dicBig) {
            ...
            }else if([[dicBig objectForKey:@"code"] intValue] ==-7){
                [SYObject failedPrompt:@"购买数量必须大于商品的最低购买数量"];
                
            }else if([[dicBig objectForKey:@"code"] intValue] ==-8){
                [SYObject failedPrompt:@"购买数量必须小于商品的最高购买数量"];

            }else if([[dicBig objectForKey:@"code"] intValue] ==-9){
                [SYObject failedPrompt:@"当前用户等级低于该商品购买等级"];
                
            }else if([[dicBig objectForKey:@"code"] intValue] ==-10){
                [SYObject failedPrompt:@"商品店铺已停业"];
...
#pragma mark -该用户无法继续购买，，真正的立即购买请求
//真正的立即购买请求
    ...
    if (statuscode2 == 200) {
        ...
        if (dicBig) {
            if ([[dicBig objectForKey:@"code"] intValue] ==0) {
                ...
                [SYObject failedPrompt:@"组合配件添加"];
            }else if([[dicBig objectForKey:@"code"] intValue] ==2) {
                [SYObject failedPrompt:@"组合套装添加"];
            }else if([[dicBig objectForKey:@"code"] intValue] ==-1) {
                [SYObject failedPrompt:@"添加失败"];
            }else if([[dicBig objectForKey:@"code"] intValue] ==-2) {
                [SYObject failedPrompt:@"商品下架"];
            }else if([[dicBig objectForKey:@"code"] intValue] ==-3) {
                [SYObject failedPrompt:@"库存不足"];
            }else if([[dicBig objectForKey:@"code"] intValue] ==-4) {
                [SYObject failedPrompt:@"为超过限购数量，添加失败"];
            }else if([[dicBig objectForKey:@"code"] intValue] ==-5) {
                [SYObject failedPrompt:@"为限购商品，请登录"];
            }else if([[dicBig objectForKey:@"code"] intValue] ==-7) {
                [SYObject failedPrompt:@"购买数量必须大于商品的最低购买数量"];
            }else if([[dicBig objectForKey:@"code"] intValue] ==-8) {
                [SYObject failedPrompt:@"购买数量必须小于商品的最高购买数量"];
...
-(void)singleTap{
    ...
    for(int i=0;i<arrImage.count;i++){
        ...
        [imageVIew sd_setImageWithURL:[NSURL URLWithString:[arrImage objectAtIndex:i]] placeholderImage:[UIImage imageNamed:@"nouse-mlh_lj"]];
...
#pragma mark -保存购买的数量
            ...
            NSUserDefaults *d=[NSUserDefaults standardUserDefaults];
            [d setObject:countField.text forKey:@"buygoodcount"];
            [d synchronize];
            countField1.text=countField.text;
            [MyTableView reloadData];
        }
...
//                            FBuyView.hidden = NO;
                            ...
                            FCode_bottomView.hidden = YES;
                            FBuyView.hidden = YES;
#pragma mark -这里先清除购物车中的购物列表
                            
//                            [self cleanGoodCarByGoodID:sec.detail_id];
                            
                            //删除的是已登陆的
#pragma mark - 获取goods_id对应的cart_id

                            NSString *cart_id=nil;
                            for (NSDictionary * d in _cart_ids) {
                                NSString *goodID=[NSString stringWithFormat:@"%@",d[@"goods_id"]];
                                if (goodID.intValue== sec.detail_id.intValue) {
                                    cart_id=[NSString stringWithFormat:@"%@",d[@"cart_id"]];
                                }
                            }
                            
                            if (cart_id==nil) {//如果没有加入过购物车
                                ...
                                if ([status isEqualToString:@"促销"]) {
                                    ...
                                    NSLog(@"price===222==%@",classifyKu.detail_goods_current_price );

                                }else{
                                    [request_46 setPostValue:classifyKu.detail_goods_current_price forKey:@"price"];
                                    
                                    
                                    NSLog(@"price===11==%@",classifyKu.detail_goods_current_price );
                                }
                                if (specSelectArray.count!=0) {
                                    NSString *str;
                                    for(int i=0;i<specSelectArray.count;i++){
                                ...
                                NSString *urlStra =[NSString stringWithFormat:@"%@%@",FIRST_URL,DELETECAR_URL];
                                NSArray *fileContent2 = [MyUtil returnLocalUserFile];
                                
                                NSDictionary *par=@{@"user_id":[fileContent2 objectAtIndex:3],@"token":[fileContent2 objectAtIndex:1],@"cart_mobile_ids":@"",@"cart_ids":cart_id};
                                __weak typeof(self) ws= self;
                                [[Requester managerWithHeader]POST:urlStra parameters:par success:^(AFHTTPRequestOperation *operation, id responseObject) {
                                    NSDictionary *dic = responseObject;
//                                    NSLog(@"======%@",dic);[self getCardIDArr];
#if 1
                                    //发起轻松购请求
                                    [SYObject startLoading];
                                    NSString *readPath2 = [documentsPath stringByAppendingPathComponent:@"information.txt"];
                                    ...
                                    if ([status isEqualToString:@"促销"]) {
                                        ...
                                        NSLog(@"price---666==%@",[NSString stringWithFormat:@"%0.2f",[classifyKu.detail_goods_current_price floatValue]* [rate1 floatValue]]);
                                    }else{
                                        [request_46 setPostValue:classifyKu.detail_goods_current_price forKey:@"price"];
                                    }
                                    if (specSelectArray.count!=0) {
                        ...
                        if ([classifyKu.detail_advance_ding_pay isEqualToString:@"0"]) {
                            ...
                            if ([status isEqualToString:@"促销"]) {
                                ...
                                [request_46 setPostValue:[NSString stringWithFormat:@"%0.2f",[classifyKu.detail_goods_current_price floatValue]* [rate1 floatValue]] forKey:@"price"];
                                  ...
                                  NSLog(@"price ===898948==%@",[NSString stringWithFormat:@"%0.2f",[classifyKu.detail_goods_current_price floatValue]* [rate1 floatValue]]) ;
                            }else{
//                                [request_46 setPostValue:classifyKu.detail_goods_current_price forKey:@"price"];
                                 [request_46 setPostValue:classifyKu.detail_advance_ding forKey:@"price"];
                                
                                NSLog(@"price ===898==%@",classifyKu.detail_advance_ding);
...
#pragma mark -获取购物车ID
-(void)getCardIDArr{
    NSString *url = [NSString stringWithFormat:@"%@%@",FIRST_URL,MYCAR_URL];
      NSArray *fileContent2 = [MyUtil returnLocalUserFile];
    
    if(fileContent2==nil){
        return;
    }
    NSMutableDictionary *par=@{@"user_id":[fileContent2 objectAtIndex:3],@"token":[fileContent2 objectAtIndex:1]}.mutableCopy;
    
    if (_cart_ids) {
        [par setObject:_cart_ids forKey:@"selected_ids"];

    }
    [[Requester managerWithHeader]POST:url parameters:par success:^(AFHTTPRequestOperation *operation, id responseObject) {
        NSDictionary *dic = responseObject;
        if (dic[@"cart_list"]) {
            _cart_ids=dic[@"cart_list"];
...
#pragma mark -清楚购物车的方法
-(void)cleanGoodCarByGoodID:(NSString *)goods_id{
    //删除的是已登陆的
#pragma mark - 获取goods_id对应的cart_id
//    NSString *cart_id=nil;
//    for (NSDictionary * d in _cart_ids) {
//        NSString *goodID=[NSString stringWithFormat:@"%@",d[@"goods_id"]];
//        if (goodID.intValue== goods_id.intValue) {
//            cart_id=goodID;
//        }
//    }
//    
...
#pragma mark - viewWillAppear数据给弄成1
    NSUserDefaults *d=[NSUserDefaults standardUserDefaults];
    [d setObject:@"1" forKey:@"buygoodcount"];
#pragma mark -获取购物车数组
    [self getCardIDArr];
