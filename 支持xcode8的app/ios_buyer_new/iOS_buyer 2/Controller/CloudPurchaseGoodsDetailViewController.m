...
-(NSArray *)DataTypeConversion:(NSArray *)datArray{
    ...
    for ( CloudPurchaseGoodsModel *model in datArray) {
        ...
        classify.detail_goods_photos_small=model.primary_photo;
...
#pragma mark -明烙华修改
                ...
                if ([[ary1 objectAtIndex:i]componentsSeparatedByString:@"\""].count>2) {
...
-(void)getNetworking2
{
    ...
    [[Requester managerWithHeader]POST:urlStr parameters:par success:^(AFHTTPRequestOperation *operation, id responseObject) {
        ...
        if(code ==10001)
        {
            ...
            [ary1 removeLastObject];
            for (int i=0; i<ary1.count; i++) {
                NSString *s=[ary1 objectAtIndex:i];
                if (s.length>0) {
                    NSString *img=[[s componentsSeparatedByString:@"\""]objectAtIndex:1];
                    [arrImage addObject:img];
                }
