...
@interface writeViewController ()<UIScrollViewDelegate>
{
    ...
    int needValidate;//判断是否需要验证地址中的身份证校验。0表示不用，1表示需要验证
    int hasCard;//有没有身份证，0表示无，1表示有身份证
    BOOL isfirstAppear;
}
...
//    [self settingAddressRequest];
    [self getAddress];
    
}
#pragma mark viewwillappear的地址请求
-(void)getAddress{
   ...
   NSString *urlstr =[NSString stringWithFormat:@"%@%@",FIRST_URL,DEFAULTADDRESSFIND_URL];
...
#pragma mark 如果有收获的默认地址
            
            __weak typeof(self) weakself=self;
            if ([[dicBig allKeys] containsObject:@"addr_id"]){

                if ([[dicBig objectForKey:@"card"] length]>0) {
                    hasCard=1;
                }else{
                    hasCard =0;
                }
...
#pragma mark 如果没有的默认地址
                
            }else{
                mybool = NO;
                third.person_address=@"";
                hasCard=0;
//                
            }
        }
...
#pragma mark 设置获取地址列表的请求
-(void)settingAddressRequest{
...
-(void)goodsListRequest
{
    ...
    [[Requester managerWithHeader]POST:urlStr parameters:par success:^(AFHTTPRequestOperation *operation, id responseObject) {
        ...
        for (NSDictionary *dic in arr) {
            ...
            clas.channel_type=[dic objectForKey:@"channel_type"];
            [goodsListArray addObject:clas];
        }
       
        [_MyTableView reloadData];
#pragma mark 第一个viewiddload的时候要获取一下地址列表，然后如果没有地址的话，需要跳到新页面编辑，此时在viewwillappear再一次获取地址列表
        [weakself validate];//验证有没有海外或者是保税的
        [weakself settingAddressRequest];
    } failure:^(AFHTTPRequestOperation *operation, NSError *error) {
...
#pragma  mark 添加海外或者是保税校验
-(void)validate{


//    判断有没有商品是海外或者是海外的，如果是，那么就要验证地址中有没有身份校验过；
//    channel_type
//    / 发货通道 2:海外直购，1:保税发货，0:国内发货
    for (ClassifyModel *obj in goodsListArray) {
        NSString *str=[NSString stringWithFormat:@"%@",obj.channel_type];
        if(![str isEqualToString:@"0"]){//1或者是2
          //只要有一个是1或者2，就要校验。
#pragma mark -改成0都不需要验证
             needValidate=1;
//            needValidate=0;
...
#pragma mark - 取得
    ...
    for (UIViewController *obj in controllers) {
        if ([obj isKindOfClass:[ThirdViewController class]]) {
            NSUserDefaults *d=[NSUserDefaults standardUserDefaults];
            count= [d valueForKey:@"buygoodcountFromBuyCar"];
            NSLog(@"countcountcountcount111=%@",count);
            break;
        }
    }
    ...
    if(count==nil){
        NSUserDefaults *d=[NSUserDefaults standardUserDefaults];
        count= [d valueForKey:@"buygoodcount"];
        NSLog(@"countcountcountcount22=%@",count);
    }
...
#pragma mark --如果提交不成功的话，这里加了物流的价格，导致双倍价格出现。改下顺序就成功了。－－明烙华修改
        
        
        if(third.person_address.length==0){
            
            [SYObject failedPrompt:@"请选择收货地址"];

        }else if (needValidate ==1 && hasCard==0) {
            [SYObject failedPrompt:@"请选择经过身份证校验的收货地址"];

        }else if (third.pay.length ==0  ||[third.pay isEqualToString:@"请选择支付方式"]) {
            [SYObject endLoading];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (indexPath.row == 4) {
#if 1   
//        这里原来有一个发票的选项的，我在xib里面把发票给隐藏了，把约束的高改成0了
        ...
        cell.shipPrice.text = [NSString stringWithFormat:@"+￥%@",third.wuliu];//物流价格
        NSLog(@"物流");
...
#pragma mark -如果是从购物车进来的话就可以直接取出数量，明烙华注释以下代码
//        NSString *count=nil;
//        for (UIViewController *obj in controllers) {
//            if ([obj isKindOfClass:[ThirdViewController class]]) {
//                NSUserDefaults *d=[NSUserDefaults standardUserDefaults];
...
//        
        ...
        if ((float)third.jie_reduce>0) {
#pragma mark -这里我做了修改
            float goodprice=[beforePrice floatValue];

            cell.goodsPrice.text =[NSString stringWithFormat:@"+￥%0.2f",goodprice] ;
        }else{
            
            
#pragma mark -这里我做了修改就是，＊个数了，为了测试先取消count
//商品总价格，不包括物流
            float goodprice=[third.jie_order_goods_price floatValue];
...
//        [a setObject:_realPrice.text forKey:@"realPrice"];
//        [a synchronize];
        if(third.jie_reduce == 0){
...
-(void)mmurlRequestSucceeded:(ASIFormDataRequest *)request{
    ...
    if (statuscode2 == 200) {
        ...
        NSData *jsonData = [NSJSONSerialization dataWithJSONObject:dicBig options:NSJSONWritingPrettyPrinted error:nil];
        NSLog(@"dicBig:%@",[[NSString alloc] initWithData:jsonData encoding:NSUTF8StringEncoding]);
...
#pragma mark - 支付配送那里
            if (strship_id.length == 0) {
                ...
                third.wuliu=strship_id;

                NSLog(@"111111=%@",strship_id);
            }else{
                ...
                NSArray *numArr=[strship_id componentsSeparatedByString:@","];
                float wuliu=0;
                for (NSString *obj in numArr) {
                    wuliu+=obj.floatValue;
                }
                
                third.wuliu=[NSString stringWithFormat:@"%.2f",wuliu];
                NSLog(@"22222=%@",strship_id);
...
//        ****************以下是明烙华添加的**************************
                NSString *string =[dicBig objectForKey:@"telephone"];
                NSString *phone=nil;
                if(string.length == 0){
                    phone =[NSString stringWithFormat:@"%@",[dicBig objectForKey:@"mobile"]];
                }else{
                    phone =[NSString stringWithFormat:@"%@",[dicBig objectForKey:@"telephone"]];
                }
                //如果有身份证，，如果没有身份证，
//                NSString *ID=@"";
                if ([[dicBig objectForKey:@"card"] length]>0) {
                    hasCard=1;
//                    ID=[dicBig objectForKey:@"card"];
                }else{
                ...
                if (needValidate ==0 ) {//不用验证身份证
                    
                }else if(needValidate==1 && hasCard==0){//要验证身份证
                    NSLog(@"要验证身份证");
                    
//                    //获取信息传到下一个控制器
//                    NSArray *arr= @[[dicBig objectForKey:@"addr_id"],[dicBig objectForKey:@"area"],[dicBig objectForKey:@"areaInfo"],phone,[dicBig objectForKey:@"trueName"],[dicBig objectForKey:@"zip"],ID];
                    
                    UIAlertController *alertCon=[UIAlertController alertControllerWithTitle:@"" message:@"您当前购买的商品包含了保税仓发货或者是海外直邮的产品，请选择经过身份证验证的收货地址" preferredStyle:UIAlertControllerStyleAlert];
                    
                    UIAlertAction *action=[UIAlertAction actionWithTitle:@"确定" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
                        
                        NSLog(@"state=%d 跳到地址编辑页面",needValidate);
                        AddAdminViewController *list = [[AddAdminViewController alloc]init];
//                        edit.editType=SY_ADDRESS_POST_TYPE_EDIT;
//                        edit.infoArrayWhenEditing=arr;
                        list.state=needValidate;
...
#pragma mark 如果没有的默认地址
                ...
                if (needValidate==1) {
                    message=@"注:您当前购买的商品包含了保税仓发货或者是海外直邮的产品,请选择经过身份证验证的收货地址";
                }else{
                    
                }
                UIAlertController *alertCon=[UIAlertController alertControllerWithTitle:@"请选择收货地址" message:message preferredStyle:UIAlertControllerStyleAlert];
