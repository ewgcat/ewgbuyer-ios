...
-(void)getData{
    ...
    NSDictionary *par=@{@"user_id":[fileContent2 objectAtIndex:3],@"token":[fileContent2 objectAtIndex:1],@"begincount":str ,@"selectcount":@"10",@"select_user_id":[fileContent2 objectAtIndex:3]};
//
    ...
    [[Requester managerWithHeader]POST:urlStr parameters:par success:^(AFHTTPRequestOperation *operation, id responseObject) {
        ...
        label2.text=[NSString stringWithFormat:@"梦想e+"];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    NSDictionary *dic=self.dataArr[indexPath.row];
    NSString *a=[NSString stringWithFormat:@"%@",dic[@"child_size"]];
    if ([a isEqualToString:@"0"]) {
        return;
    }
    SubTeamTableViewController *vc=[[SubTeamTableViewController alloc]init];
    
    vc.userId=dic[@"user_id"];
