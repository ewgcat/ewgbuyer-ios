...
- (BOOL)gestureRecognizerShouldBegin:(UIGestureRecognizer *)gestureRecognizer{
    ...
    NSLog(@"viewControllers22222===%@",self.navigationController.viewControllers);
...
- (void)viewDidLoad
{
    ...
    if ([[NSString stringWithFormat:@"%@",sec.sub_id2] isEqualToString:@"search"]) {
        [self NetWork_OrderBy:@"goods_salenum" orderType:@"desc" gc_id:@"" keyword:sec.searchKeyword beginCount:@"0" selectCount:@"20" properties:@""];
    }else{
        NSLog(@"====%@",sec.sub_id2);
...
-(NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section{
    ...
    if (tableView == MyTableView) {
        ...
        if (FilterDataArray.count!=0 && FilterFirstIndex>=0) {
...
//    loadingV.hidden = NO;
    ...
    if (btn.tag == 102) {
        ...
        if ([[NSString stringWithFormat:@"%@",sec.sub_id2] isEqualToString:@"search"]) {
            [self NetWork_OrderBy:@"goods_salenum" orderType:@"desc" gc_id:@"" keyword:sec.searchKeyword beginCount:@"0" selectCount:@"20" properties:[self properties_str]];
        }else{
            [self NetWork_OrderBy:@"goods_salenum" orderType:@"desc" gc_id:sec.sub_id2 keyword:@"" beginCount:@"0" selectCount:@"20" properties:[self properties_str]];
        }
    }
    if (btn.tag == 103) {
        priceImage.image = [UIImage imageNamed:@"price_gray"];
...
-(void)NetWork_OrderBy:(NSString *)orderBy orderType:(NSString *)orderType gc_id:(NSString *)gc_id keyword:(NSString *)keyword beginCount:(NSString *)beginCount selectCount:(NSString *)selectCount properties:(NSString *)properties {
    ...
    if ([fileContent objectAtIndex:3] ) {
        [request101 setPostValue:[fileContent objectAtIndex:3] forKey:@"user_id"];

    }else{
        [request101 setPostValue:@"" forKey:@"user_id"];

    }
    if ([fileContent objectAtIndex:1]) {
        [request101 setPostValue:[fileContent objectAtIndex:1] forKey:@"token"];

    }else{
        [request101 setPostValue:@""forKey:@"token"];
