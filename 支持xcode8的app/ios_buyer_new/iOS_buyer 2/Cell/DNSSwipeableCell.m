...
- (UIButton *)buttonForIndex:(NSInteger)index previousButtonMinX:(CGFloat)previousMinX inCellAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([self.dataSource respondsToSelector:@selector(widthForButtons)]) {
        bW = [self.dataSource widthForButtons];
    }
    if (bW == 0) {
        ...
        if (xOrigin < 40) {
            NSLog(@"***ATTENTION!*** Button at index %ld at index path %@ is going to leave less than 40 points of space! That's going to be hard to close.", (long)index, indexPath);
        }
    }else{
        CGFloat xOrigin = previousMinX - bW;
        button.frame = CGRectMake(xOrigin, 0, bW, CGRectGetHeight(self.frame));
        if (xOrigin < 40) {
            NSLog(@"***ATTENTION!*** Button at index %ld at index path %@ is going to leave less than 40 points of space! That's going to be hard to close.", (long)index, indexPath);
        }
    }
